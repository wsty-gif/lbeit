...
class SearchForm {
  ...
  async render(){
    ...
    this.el.querySelector("#sf-key").addEventListener("input", e=>{ this.state.keyword = e.target.value.trim(); });
  ...
  openModal(key){
    document.querySelector(`#modal-${key}`).classList.add("show");
    document.querySelectorAll(`#modal-${key} [data-close]`).forEach(b=> b.onclick = ()=>{
  ...
  buildLocationModal(){
    ...
    regionMenu.innerHTML = regions.map((r,i)=>`<li><button class="side-btn ${i===0?"active":""}" data-region="${r}">${r}</button></li>`).join("");
      ...
      prefWrap.querySelectorAll(".toggle").forEach(btn=>{
        btn.addEventListener("click", ()=>{
          ...
          if(isHidden){
            ...
            list.innerHTML = Object.keys(cities).map(c=>{
              const wards = cities[c];
              const hasWards = Array.isArray(wards) && wards.length > 0;
              return `
                <div class="city-block">
                  <div class="city-head">
                    <label class="opt">
                      <input class="checkbox" type="checkbox" data-loc="${pref}/${c}"> ${c}
                    </label>
                    ${hasWards?`<button class="toggle" data-city="${c}">＋</button>`:""}
                  </div>
                  ${hasWards?`<div class="inner hidden" data-ward-list="${c}">
                    ${wards.map(w=>`<label class="opt"><input class="checkbox" type="checkbox" data-loc="${pref}/${c}/${w}"> ${w}</label>`).join("")}
                  </div>`:""}
                </div>
              `;
            }).join("");
  ...
  buildJobModal(){
    ...
    body.innerHTML = `
      ...
      <div id="${listId}" class="job-list">
        ${this.ds.jobCategories.map(j=>`
          <label class="opt"><input class="checkbox job-chk" type="checkbox" value="${j}"> ${j}</label>
        `).join("")}
      </div>
    `;
...
// PREF_CITY_DATA は別ファイル(prefCities.js)で定義済み
