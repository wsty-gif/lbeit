class SearchForm {
  ...
  async render() {
    ...
    const pillLbl = "inline-flex items-center gap-2 text-sm border rounded-full px-3 py-2 cursor-pointer bg-white hover:bg-gray-50";
      ...
      wrap.innerHTML = prefs.map(pref=>{
        ...
        return `
          <div class="border rounded-lg overflow-hidden">
            ...
            <div class="p-3">
              <div class="max-h-56 overflow-y-auto grid grid-cols-2 sm:grid-cols-3 gap-x-4">
                ${cities.map(city=>`
                ...
                `).join("") || `<div class="text-xs text-gray-500">市区町村データなし</div>`}
              </div>
            </div>
          </div>
        `;
    ...
    const drawJobs = (q="") => {
      ...
      wrap.innerHTML = items.map(c=>`
        <label class="inline-flex items-center gap-2 border rounded-lg px-3 py-2 text-sm bg-white hover:bg-gray-50">
    ...
    const drawPrefs = (q="") => {
      ...
      prefBody.innerHTML = `
        <div class="space-y-3">
          <input id="pref-free" class="w-full h-12 rounded-lg border border-gray-300 px-3 bg-white focus:outline-none focus:ring-2 focus:ring-orange-300" placeholder="こだわりをフリーワードで探す">
